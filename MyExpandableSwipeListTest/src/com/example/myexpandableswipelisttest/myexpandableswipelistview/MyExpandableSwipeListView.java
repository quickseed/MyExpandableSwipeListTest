...
import com.example.myexpandableswipelisttest.R;
 ...
 */
public class MyExpandableSwipeListView extends ExpandableListView {
	...
	/**
	 * Default id for group view
	 */
	public final static String SWIPE_DEFAULT_GROUP_VIEW = "swipelist_groupview";
	 ...
	 *            Front Identifier
	 */
	public MyExpandableSwipeListView(Context context, int swipeBackView,
			int swipeFrontView, int swipeGroupView) {
		super(context);
	 ...
	 */
	public MyExpandableSwipeListView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(attrs);
	}
	...
	private void init(AttributeSet attrs) {
		...
		if (attrs != null) {
			TypedArray styled = getContext().obtainStyledAttributes(attrs,
					R.styleable.MyExpandableSwipeListView);
			swipeMode = styled.getInt(
					R.styleable.MyExpandableSwipeListView_swipeMode,
					SWIPE_MODE_BOTH);
			swipeActionLeft = styled.getInt(
					R.styleable.MyExpandableSwipeListView_swipeActionLeft,
					SWIPE_ACTION_REVEAL);
			swipeActionRight = styled.getInt(
					R.styleable.MyExpandableSwipeListView_swipeActionRight,
					SWIPE_ACTION_REVEAL);
			swipeOffsetLeft = styled.getDimension(
					R.styleable.MyExpandableSwipeListView_swipeOffsetLeft, 0);
			swipeOffsetRight = styled.getDimension(
					R.styleable.MyExpandableSwipeListView_swipeOffsetRight, 0);
			swipeOpenOnLongPress = styled.getBoolean(
					R.styleable.MyExpandableSwipeListView_swipeOpenOnLongPress,
					true);
			swipeAnimationTime = styled
					.getInteger(
							R.styleable.MyExpandableSwipeListView_swipeAnimationTime,
							0);
			swipeCloseAllItemsWhenMoveList = styled
					.getBoolean(
							R.styleable.MyExpandableSwipeListView_swipeCloseAllItemsWhenMoveList,
							true);
			swipeDrawableChecked = styled.getResourceId(
					R.styleable.MyExpandableSwipeListView_swipeDrawableChecked,
					0);
			swipeDrawableUnchecked = styled
					.getResourceId(
							R.styleable.MyExpandableSwipeListView_swipeDrawableUnchecked,
							0);
			swipeFrontView = styled.getResourceId(
					R.styleable.MyExpandableSwipeListView_swipeFrontView, 0);
			swipeBackView = styled.getResourceId(
					R.styleable.MyExpandableSwipeListView_swipeBackView, 0);
			swipeGroupView = styled.getResourceId(
					R.styleable.MyExpandableSwipeListView_swipeGroupView, 0);
		}

		if (swipeFrontView == 0 || swipeBackView == 0 || swipeGroupView == 0) {// jin
			...
			swipeBackView = getContext().getResources().getIdentifier(
					...
					getContext().getPackageName());
			swipeGroupView = getContext().getResources().getIdentifier(
					SWIPE_DEFAULT_GROUP_VIEW, "id",
					getContext().getPackageName());// jin

			if (swipeFrontView == 0 || swipeBackView == 0
					|| swipeGroupView == 0) {// jin
				throw new RuntimeException(
						String.format(
								"You forgot the attributes swipeFrontView or swipeBackView,swipeGroupView. You can add this attributes or use '%s' and '%s' identifiers",
								SWIPE_DEFAULT_FRONT_VIEW,
								SWIPE_DEFAULT_BACK_VIEW,
								SWIPE_DEFAULT_GROUP_VIEW));
			}
		}
		...
		touchListener = new ExpandableSwipeListViewTouchListener(this,
				swipeFrontView, swipeBackView, swipeGroupView);
	...
	protected void onStartOpen(int position, int action, boolean right) {
		if (swipeListViewListener != null
				&& position != ExpandableListView.INVALID_POSITION) {
	...
	// jin
	/**
	 * Returns the group and child positions for a child element. This values
	 * are passed inside an array of dimension 2 where the index 0 is the group
	 * position and the index 1 is the child position.
	 * 
	 * @param general_position
	 *            used on the list (compatible with getChildAt)
	...
	public int[] getPositionArray(int general_position) {
		int groupReadedNum = 0;// 遍历过的group数目
		int childReadednum = 0;// 遍历过的child数目
		int groupPosition = 0;
		int childPosition = 0;
		View child;
		for (int i = 0; i < general_position; i++) {
			child = this.getChildAt(i);
			if (child.findViewById(swipeGroupView) != null) {
				groupReadedNum++;
			}
		}
			groupPosition = groupReadedNum- 1;
		for (int i = 0; i < groupPosition; i++) {
			if(isGroupExpanded(groupPosition)){			
				childReadednum += getExpandableListAdapter().getChildrenCount(i);
			}
		}
		childPosition = general_position - childReadednum - groupReadedNum;
		return new int[] { groupPosition, childPosition };
	}

	/**
	 * Returns the general position of an element on the list (used by
	 * getChildAt)
	 * 
	 * @param groupPosition
	 * @param childPosition
